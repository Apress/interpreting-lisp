...
----------------------------------------------------------------------------*/
{
   ...
   t = type(r);
   if (namedfsf(t))
      r = ptrv(Atab[ptrv(r)].L); /* dereference r */
   if (builtin(t))
      r = nilptr; /*do not try to mark a builtin */
   gcmark(r);
   gc();
...
#define skp Atab[sk].L
   ...
   strcpy(g, "@lispinit ");
   pg = g;
   pge = g + strlen(g); /* initialize start & end pointers to string g */
   filep = stdin;
}
...
start:
   while ((c = getgchar()) EQ BLANK)
      ; /* remove blanks */

   if (c EQ OPENP)
   {
      while (lookgchar() EQ BLANK)
...
#define LF 10
   ...
   {
      if ((c EQ TAB)OR(c EQ CR))
         c = BLANK;
      s[i] = c;
   }
   s[i] = '\0';
   if ((c EQ EOF)AND(i EQ 0))
      return (-1);
   else
      return (i);
}
...
#define Return(v)       \
   ...
   {             /* f is a non-builtin function or non-builtin special form.  do
    ...
    value of each formal argument on its binding list. */
      ...
      if (p != nilptr)
         error("too many actual argumentss");
   ...
   { /* at this point we have a builtin function or special form.  f
      ...
      case 2: /* CDR */
         ...
         break;
      case 3: /* CONS */
         if (sexp(type(E1)) AND sexp(type(E2)))
            v = newloc(E1, E2);
         else
      ...
      case 7: /* ATOM */
         if ((type(E1))EQ 8 OR(type(E1)) EQ 9)
            v = tptr;
         break;
      ...
      case 24: /* AND */
         while (p != nilptr AND seval(A(p)) != nilptr)
            p = B(p);
         if (p EQ nilptr)
      ...
      case 25: /* OR */
         while (p != nilptr AND seval(A(p)) EQ nilptr)
            p = B(p);
         if (p != nilptr)
            v = tptr; /* else v remains nilptr */
         break;
      ...
      case 37: /* TSETQ */
         ...
         if (Atab[f = ptrv(U1)].bl EQ nilptr)
            goto assign;
         v = Atab[f].bl;
         while (B(v) != nilptr)
            v = B(v);
      ...
      default:
         error("dryrot: bad builtin case number");
      } /* end of switch cases */
